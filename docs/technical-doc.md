...
- **`npm` unconditionally strips any directory literally named `node_modules`, anywhere in the package — regardless of `files`.** Next.js 16 defaults to Turbopack for both `next dev` and `next build`. Turbopack's production build traces certain dependencies that can't be bundled into a JS chunk (native addons like `better-sqlite3`, and `@prisma/client`) and physically copies each one into `.next/node_modules/<content-hash>/`, with the compiled server chunks `require()`-ing them by that hashed name (relying on Node's ordinary `node_modules`-resolution walk finding that directory as an ancestor of the requiring chunk). Since `files` can't override npm's hardcoded `node_modules` exclusion, `.next/node_modules` silently vanished from every published tarball regardless of what `files` said, and the server crashed on the first request that touched the database with `Cannot find module '<hashed-name>'` — again on every OS, first caught via a Windows user's report.
  - **Abandoned approach — building with `--webpack`**: Next 16 still ships webpack as an explicit opt-out (`next build --webpack`) from the new Turbopack default, and its `serverExternalPackages` docs describe the intended behavior for packages like `better-sqlite3`/`@prisma/client` as a plain, unhashed native `require()` — which looked like it would sidestep the whole vendor-copy problem at the source. It was tried and published as v0.1.2, but broke something *worse*: Prisma's generated `src/generated/prisma/client.ts` computes its own `__dirname` via `path.dirname(fileURLToPath(import.meta.url))`, and webpack's CommonJS output statically replaces `import.meta.url` with a **literal absolute path computed at build time** rather than resolving it dynamically at runtime like real ESM does. That baked the *maintainer's own local machine's* absolute path (e.g. `/Users/.../resume-agent/src/generated/prisma/client.ts`) into the shipped bundle. It "worked" in every test run on the machine that built it (the baked-in path happened to still be correct there) and only broke once installed on a *different* machine — first surfaced by a Windows user's `TypeError: File URL path must be absolute` (`ERR_INVALID_FILE_URL_PATH`), which is Node's Windows-specific `fileURLToPath` validation rejecting a bare Unix-style path with no drive letter. Reverted; `prepublishOnly` is back to plain `next build` (Turbopack).
  - **Attempt 2 — dereference the symlinks, don't just rename them**: the original relocate/restore approach (rename `.next/node_modules` → `.next/vendor`, rename back at first launch) turned out to still be broken, for a reason a plain filesystem-level rename test never caught: the hashed entries under `.next/node_modules/` (`better-sqlite3-<hash>`, `@prisma/client-<hash>`) are themselves **symlinks into the real top-level `node_modules/`** (Turbopack vendors this way specifically to avoid duplicating large native binaries on disk), not real copies. A rename just moves the symlink — it still *resolves* into `node_modules/`, and npm's packer excludes anything whose resolved path contains a `node_modules` path segment, not just things literally named that at the top level. Switched `scripts/relocate-next-native-deps.mjs` to `fs.cpSync(from, to, { recursive: true, dereference: true })` — follows symlinks and copies their real target content — instead of `fs.renameSync`. Verified end-to-end (`npm pack`, installed into a throwaway `--prefix`, launched with a fake `$HOME` and a `cwd` outside the repo, every route hit cleanly) and published as v0.1.3.
  - **v0.1.3 broke on Windows anyway — because same-machine, same-OS verification can't catch a cross-OS bug**: `better-sqlite3` ships a compiled native `.node` binary specific to one OS/architecture. Dereferencing the symlink copies *the maintainer's own machine's compiled binary* into the published package — on this repo that means a macOS binary. It loads fine on macOS (which is all the "verified end-to-end" pass above could ever have tested, run on the maintainer's own Mac), and fails everywhere else with an OS-loader error — on Windows, `Invalid \`prisma.personalInfo.findFirst()\` invocation: ...\better_sqlite3.node is not a valid Win32 application`. The lesson: an end-to-end install test is only as strong as the diversity of machines it actually runs on — "verified for real" on one platform does not mean cross-platform-safe when the thing being shipped is platform-specific by nature.
  - **The actual correct fix — don't ship any file content, ship a map**: `better-sqlite3` and `@prisma/client` are already real dependencies of `resume-agent` — a normal `npm install` already fetches the *platform-correct* build for whoever is installing it, into their own `node_modules/`. The only real problem was ever that the compiled output looks for it under a hashed name Node can't resolve unassisted. So `scripts/relocate-next-native-deps.mjs` no longer copies any binary content at all: it walks `.next/node_modules/`, resolves each symlink via `fs.realpathSync`, records the mapping from hashed name to real package path (e.g. `{"better-sqlite3-<hash>": "better-sqlite3", "@prisma/client-<hash>": "@prisma/client"}`) into a tiny `.next/vendor-map.json`, then deletes `.next/node_modules/` outright (npm would have stripped it anyway). `bin/resume-agent.js`'s `ensureNativeVendorModules()` reads that manifest at first launch and creates a fresh symlink (`fs.symlinkSync`, using `'junction'` type on Windows so it doesn't require elevated privileges) from the hashed path straight into `<PACKAGE_ROOT>/node_modules/<real package>` — the end user's own, already platform-correct, install. As a side benefit, package size dropped back to ~16.8 MB compressed (from v0.1.3's ~48.5 MB), since nothing gets duplicated anymore. Re-verified with the same end-to-end install methodology as before (confirming the fix mechanism is sound), plus direct inspection that the resulting symlink target on macOS is the real local `node_modules/better-sqlite3` rather than any copied content — the actual cross-platform claim rests on the fact that this now defers entirely to npm's own per-machine native-module installation rather than on having tested it on Windows directly.
