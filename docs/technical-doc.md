...
- **`npm` unconditionally strips any directory literally named `node_modules`, anywhere in the package — regardless of `files`.** Next.js 16 defaults to Turbopack for both `next dev` and `next build`. Turbopack's production build traces certain dependencies that can't be bundled into a JS chunk (native addons like `better-sqlite3`, and `@prisma/client`) and physically copies each one into `.next/node_modules/<content-hash>/`, with the compiled server chunks `require()`-ing them by that hashed name (relying on Node's ordinary `node_modules`-resolution walk finding that directory as an ancestor of the requiring chunk). Since `files` can't override npm's hardcoded `node_modules` exclusion, `.next/node_modules` silently vanished from every published tarball regardless of what `files` said, and the server crashed on the first request that touched the database with `Cannot find module '<hashed-name>'` — again on every OS, first caught via a Windows user's report.
  ...
  - **The real fix — dereference the symlinks, don't just rename them**: the original relocate/restore approach (rename `.next/node_modules` → `.next/vendor`, rename back at first launch) turned out to still be broken, for a reason a plain filesystem-level rename test never caught: the hashed entries under `.next/node_modules/` (`better-sqlite3-<hash>`, `@prisma/client-<hash>`) are themselves **symlinks into the real top-level `node_modules/`** (Turbopack vendors this way specifically to avoid duplicating large native binaries on disk), not real copies. A rename just moves the symlink — it still *resolves* into `node_modules/`, and npm's packer excludes anything whose resolved path contains a `node_modules` path segment, not just things literally named that at the top level. So the "fix" silently kept losing the same directory, just one layer more subtly (confirmed by directly inspecting the packed tarball — zero files under `.next/vendor/` made it in, even though the directory existed on disk at pack time). `scripts/relocate-next-native-deps.mjs` now uses `fs.cpSync(from, to, { recursive: true, dereference: true })` — which follows symlinks and copies their real target content — instead of `fs.renameSync`. `bin/resume-agent.js`'s `ensureNativeVendorModules()` didn't need to change; a plain rename back to `.next/node_modules` at first launch is fine once the source content is real files, not symlinks.
  - **This was verified for real, end to end**, not just by reading source: `npm pack`, installed via `npm install -g --prefix <a throwaway directory>` from a *different* tarball path, launched with a fake `$HOME` and a `cwd` outside the repo entirely (simulating a genuinely different machine), and every route hit cleanly (200s on `/`, `/profile`, `/experience`, `/generate`, `/resumes`, `/templates`, and the corresponding `/api/*` routes, plus `/api/assistant/mentionables` to confirm `.claude/skills/` resolves correctly from the installed location). The migration step, the native-dependency restore, and the server start all logged cleanly with no errors.
  - **Trade-off accepted, not yet optimized**: dereferencing copies the *entire* target package, not just the files actually needed at runtime — `@prisma/client`'s `runtime/` directory alone is ~86 MB, because it ships WASM query compilers for every database provider Prisma supports (PostgreSQL, MySQL, SQL Server, CockroachDB, SQLite), even though this app only ever uses SQLite. Published package size grew from ~16.8 MB to ~48.5 MB compressed (~52 MB → ~143 MB uncompressed) as a result. Trimming this to just the SQLite-relevant files would be a meaningful size win, but wasn't done here — after two rounds of a "cleverer" fix turning out to be broken in a way only a real end-to-end install caught, the priority was shipping something verified-correct over shipping something smaller but unverified.
