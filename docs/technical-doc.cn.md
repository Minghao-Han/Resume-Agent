...
- **npm 发布时会无条件剔除包里任何一个名字叫 `node_modules` 的目录——不管在哪一层，也不管 `files` 字段怎么写。** Next.js 16 把 `next dev` 和 `next build` 的默认打包器都换成了 Turbopack。Turbopack 的生产构建会把一些没法直接打进 JS chunk 的依赖（原生插件如 `better-sqlite3`，以及 `@prisma/client`）单独 trace 出来，物理复制一份到 `.next/node_modules/<内容哈希>/`，编译产物里用这个哈希名去 `require()` 它，靠 Node 正常的 `node_modules` 查找规则把这个目录当成 requiring chunk 的祖先目录去找到。由于 `files` 字段根本压不住 npm 这条硬编码的 `node_modules` 排除规则，`.next/node_modules` 会在每次发布时被无声剔除——不管 `files` 怎么写都没用——结果是服务一收到第一个碰数据库的请求就崩，报 `Cannot find module '<哈希名>'`。同样是所有平台都会中招，也是先从一位 Windows 用户的报告里发现的。
  - **治本的修法**：`prepublishOnly` 现在用 `next build --webpack` 来构建（Next 16 虽然把默认打包器换成了 Turbopack，但仍然保留了 `--webpack` 这个显式的退出选项）。Next 官方 `serverExternalPackages` 的文档写得很明确，`better-sqlite3`/`@prisma/client` 这类包本来的设计意图就是走普通的、不改名的原生 `require()`，靠真实的 `node_modules` 解析——这也正是经典 webpack 一直以来的做法；哈希改名复制这一套，看起来是 Turbopack 自己加的机制，不是 `serverExternalPackages` 这个特性本身要求的。**这一点我没能在本地真正跑一次构建去端到端验证**（做这个修复的时候这台机器上没有 node）——正式依赖它、丢掉下面这道保险之前，务必自己先跑一次 `rm -rf .next && npm run build`（`--webpack` 版本）确认一下。
  - **保留下来的兜底机制**：`scripts/relocate-next-native-deps.mjs`（构建后把 `.next/node_modules` 改名成 `.next/vendor`，npm 不会动这个名字）和 `bin/resume-agent.js` 的 `ensureNativeVendorModules()`（在用户机器上启动服务前把它改名改回来）都写成了"构建后如果根本没有 `.next/node_modules` 就什么也不做"——如果 `--webpack` 真的从根上解决了这个问题，这一整套机制会自动变成空操作，以后可以放心删掉；如果没解决（或者以后别的依赖触发了同样的行为），这层兜底还在，不会裸奔。
