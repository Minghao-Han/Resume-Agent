...
- **npm 发布时会无条件剔除包里任何一个名字叫 `node_modules` 的目录——不管在哪一层，也不管 `files` 字段怎么写。** Next.js 16 把 `next dev` 和 `next build` 的默认打包器都换成了 Turbopack。Turbopack 的生产构建会把一些没法直接打进 JS chunk 的依赖（原生插件如 `better-sqlite3`，以及 `@prisma/client`）单独 trace 出来，物理复制一份到 `.next/node_modules/<内容哈希>/`，编译产物里用这个哈希名去 `require()` 它，靠 Node 正常的 `node_modules` 查找规则把这个目录当成 requiring chunk 的祖先目录去找到。由于 `files` 字段根本压不住 npm 这条硬编码的 `node_modules` 排除规则，`.next/node_modules` 会在每次发布时被无声剔除——不管 `files` 怎么写都没用——结果是服务一收到第一个碰数据库的请求就崩，报 `Cannot find module '<哈希名>'`。同样是所有平台都会中招，也是先从一位 Windows 用户的报告里发现的。
  ...
  - **真正的修法——把符号链接解引用，而不是单纯改名字**：最早那版"改名搬走 / 启动时搬回"的方案，其实一直都是坏的，只是纯文件系统层面的改名测试完全测不出来：`.next/node_modules/` 下那些哈希命名的条目（`better-sqlite3-<哈希>`、`@prisma/client-<哈希>`）本身就是**指向真实顶层 `node_modules/` 的符号链接**（Turbopack 特意这么做是为了不在磁盘上重复存一份体积不小的原生二进制文件），不是真实拷贝。单纯改名只是挪动了这个符号链接本身——它还是会解析到 `node_modules/` 里面去，而 npm 打包时排除的判断依据是**解析后的路径**里有没有 `node_modules` 这一段，不只是看顶层目录字面上叫不叫这个名字。所以这个"修复"其实一直在悄悄丢失同一个目录，只是换了一层更隐蔽的方式（直接检查打包出来的 tarball 证实了这一点——即使打包那一刻 `.next/vendor/` 目录在磁盘上确实存在，里面愣是一个文件都没被打进去）。现在 `scripts/relocate-next-native-deps.mjs` 改用 `fs.cpSync(from, to, { recursive: true, dereference: true })`——会跟随符号链接、拷贝它们指向的真实内容——而不是 `fs.renameSync`。`bin/resume-agent.js` 的 `ensureNativeVendorModules()` 不需要改：源内容已经是真实文件而不是符号链接之后，启动时单纯改名改回 `.next/node_modules` 就没问题了。
  - **这次是真刀真枪端到端验证过的**，不是只靠读源码：`npm pack` 之后，通过 `npm install -g --prefix <一个随便找的临时目录>` 从打包出来的 tarball 装到一个完全独立的位置，用一个伪造的 `$HOME`、cwd 也完全在仓库目录之外启动（模拟一台真正不同的机器），然后挨个访问了所有页面（`/`、`/profile`、`/experience`、`/generate`、`/resumes`、`/templates`，以及对应的 `/api/*` 路由，外加 `/api/assistant/mentionables`——用来确认 `.claude/skills/` 确实能从装好的位置正确解析），全部干净的 200，日志里没有任何报错。
  - **接受了一个还没优化的代价**：解引用会把目标包**整个**拷贝一份，不只是运行时真正用到的那些文件——单是 `@prisma/client` 的 `runtime/` 目录就有大约 86MB，因为它把 Prisma 支持的每一种数据库（PostgreSQL、MySQL、SQL Server、CockroachDB、SQLite）对应的 WASM query compiler 全都打包了进去，而这个应用其实只用 SQLite。发布包体积因此从大约 16.8MB（压缩后）涨到了 48.5MB（未压缩从约 52MB 涨到约 143MB）。把这个裁剪到只保留 SQLite 相关的文件应该能省不少体积，但这次没做——连续两轮"更聪明"的修法结果都栽在了只有真正端到端安装才能发现的问题上，这次优先把"确认真的对"的版本发出去，而不是再去赌一个"体积更小但没验证过"的版本。
