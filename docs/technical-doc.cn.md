...
- **`bin/resume-agent.js`**（`package.json` 的 `bin` 字段指向它）是 npm 链接到 `PATH` 上的入口。每次启动都会：定位/首次创建 `~/.resume-agent/`（内含 `~/.resume-agent/storage/resumes/`）、把 `DATABASE_URL` 和 `RESUME_STORAGE_DIR` 这两个环境变量指到这里、检查是否需要跑迁移（见下一条），然后对着包里自带的 `.next` 构建产物启动 `next start`，并自动打开默认浏览器。
- **迁移检查现在靠一个标记文件（`~/.resume-agent/.last-migration`）在没有变化时直接跳过，而不是每次启动都无条件跑一遍**。`prisma migrate deploy` 本身重复跑是安全的（只会应用还没被记录为"已应用"的迁移，绝不会 drop/reset/diff 任何东西——这也是 CI/CD 每次部署都在跑的同一条命令，它本身不会导致数据丢失），但每次启动都要重新拉起 Prisma CLI 子进程、加载它的 bundle、连接数据库、比对迁移记录表，哪怕根本没有新迁移要跑也要花这些时间——对一个本该秒开的本地小工具来说这个延迟很明显。`latestMigrationName()` 直接读 `prisma/migrations/` 目录里最新的文件夹名字（不需要拉子进程），跟标记文件比对；两者一致就直接从 `runMigrations()` 返回。标记文件比对的是迁移目录的**实际内容**（不是简单的"是否迁移过"布尔值），所以新版本 `resume-agent` 带来的新迁移仍然会被自动识别到；而且这个标记只会在 `migrate deploy` 真正成功退出**之后**才写入，所以一次失败或被中断的迁移永远不可能让标记文件谎称"已经迁移过"。（已知但没有特意处理的边界情况：如果用户手动把 `~/.resume-agent/dev.db` 换成一份更旧的备份，标记文件依然会显示"已是最新"，即使换上去的这份数据库实际上落后——对一个单用户本地工具来说这是可以接受的取舍，这里没有专门去解决它。）
...
- **npm 发布时会无条件剔除包里任何一个名字叫 `node_modules` 的目录——不管在哪一层，也不管 `files` 字段怎么写。** Next.js 16 把 `next dev` 和 `next build` 的默认打包器都换成了 Turbopack。Turbopack 的生产构建会把一些没法直接打进 JS chunk 的依赖（原生插件如 `better-sqlite3`，以及 `@prisma/client`）单独 trace 出来，物理复制一份到 `.next/node_modules/<内容哈希>/`，编译产物里用这个哈希名去 `require()` 它，靠 Node 正常的 `node_modules` 查找规则把这个目录当成 requiring chunk 的祖先目录去找到。由于 `files` 字段根本压不住 npm 这条硬编码的 `node_modules` 排除规则，`.next/node_modules` 会在每次发布时被无声剔除——不管 `files` 怎么写都没用——结果是服务一收到第一个碰数据库的请求就崩，报 `Cannot find module '<哈希名>'`。同样是所有平台都会中招，也是先从一位 Windows 用户的报告里发现的。
  - **试过但放弃了的方案——用 `--webpack` 构建**：Next 16 保留了 `--webpack` 这个显式退出选项，官方 `serverExternalPackages` 文档也说 `better-sqlite3`/`@prisma/client` 这类包本该走普通的、不改名的原生 `require()`——看起来能从根上绕开整个"哈希改名复制"的问题。这个方案真的发布过一次（v0.1.2），结果捅出了一个**更严重**的问题：Prisma 生成的 `src/generated/prisma/client.ts` 用 `path.dirname(fileURLToPath(import.meta.url))` 来算自己的 `__dirname`，而 webpack 在编译成 CommonJS 输出时，会把 `import.meta.url` **静态替换成构建那一刻算出来的字面绝对路径**，而不是像真正的 ESM 那样在运行时动态解析。结果就是把**维护者自己本机**的绝对路径（比如 `/Users/.../resume-agent/src/generated/prisma/client.ts`）硬编码进了发出去的构建产物里。在构建它的那台机器上跑，怎么测都是好的（硬编码的路径碰巧就是对的），只有装到**别的机器**上才会炸——第一次是被一位 Windows 用户的 `TypeError: File URL path must be absolute`（`ERR_INVALID_FILE_URL_PATH`）报出来的，这正是 Node 在 Windows 上对 `fileURLToPath` 的校验逻辑，拒绝了一个没有盘符的类 Unix 路径。已经回退；`prepublishOnly` 改回了普通的 `next build`（Turbopack）。
  - **第二次尝试——把符号链接解引用，而不是单纯改名字**：最早那版"改名搬走 / 启动时搬回"的方案，其实一直都是坏的，只是纯文件系统层面的改名测试完全测不出来：`.next/node_modules/` 下那些哈希命名的条目（`better-sqlite3-<哈希>`、`@prisma/client-<哈希>`）本身就是**指向真实顶层 `node_modules/` 的符号链接**（Turbopack 特意这么做是为了不在磁盘上重复存一份体积不小的原生二进制文件），不是真实拷贝。单纯改名只是挪动了这个符号链接本身——它还是会解析到 `node_modules/` 里面去，而 npm 打包时排除的判断依据是**解析后的路径**里有没有 `node_modules` 这一段，不只是看顶层目录字面上叫不叫这个名字。改成 `scripts/relocate-next-native-deps.mjs` 用 `fs.cpSync(from, to, { recursive: true, dereference: true })`——跟随符号链接、拷贝真实内容——而不是 `fs.renameSync`。端到端验证过（`npm pack`、装到一个临时 `--prefix`、用伪造的 `$HOME` 和仓库外的 cwd 启动、挨个访问所有路由都是干净的 200），发布成了 v0.1.3。
  - **v0.1.3 照样在 Windows 上炸了——因为同一台机器、同一个操作系统的验证，天生就测不出跨操作系统的问题**：`better-sqlite3` 自带一个编译好的原生 `.node` 二进制文件，是绑定特定操作系统/架构的。解引用符号链接，拷贝的其实是**维护者自己这台机器编译出来的二进制文件**——这个仓库就是一份 macOS 的二进制。它在 macOS 上跑得好好的（上面那次"端到端验证"能测到的也就只有维护者自己这台 Mac），换了别的平台就必炸——在 Windows 上报的是 `Invalid \`prisma.personalInfo.findFirst()\` invocation: ...\better_sqlite3.node is not a valid Win32 application`，操作系统加载器层面的报错。这里的教训是：一次端到端安装测试的可信度，取决于它实际跑过多少台**不一样**的机器——当你发布的东西本身就是"因平台而异"的产物时，"在一个平台上真刀真枪验证过"不等于"跨平台安全"。
  - **真正正确的修法——不拷贝任何文件内容，只发一份映射表**：`better-sqlite3` 和 `@prisma/client`本来就是 `resume-agent` 的真实依赖——正常 `npm install` 本来就会给装它的这台机器拉一份**平台正确**的版本，装进它自己的 `node_modules/` 里。唯一的问题，从头到尾都只是编译产物在找一个 Node 自己解析不出来的哈希名字而已。所以现在 `scripts/relocate-next-native-deps.mjs` 完全不再拷贝任何二进制内容：遍历 `.next/node_modules/`，用 `fs.realpathSync` 把每个符号链接解析出真实指向，把"哈希名 → 真实包路径"这个映射（比如 `{"better-sqlite3-<哈希>": "better-sqlite3", "@prisma/client-<哈希>": "@prisma/client"}`）记录进一个很小的 `.next/vendor-map.json` 文件，然后把 `.next/node_modules/` 整个删掉（反正 npm 也会把它剔除）。`bin/resume-agent.js` 的 `ensureNativeVendorModules()` 在首次启动时读这份映射表，直接创建一个新的符号链接（`fs.symlinkSync`，Windows 上用 `'junction'` 类型以免需要管理员权限），从哈希路径指向 `<包根目录>/node_modules/<真实包>`——也就是这台机器自己、本来就已经装对了平台的那一份。顺带的好处是发布包体积也降回了大约 16.8MB（压缩后，比 v0.1.3 的约 48.5MB 小很多），因为不再重复拷贝任何东西。用和之前一样的端到端安装方法重新验证过（确认整套机制本身没问题），也直接检查过 macOS 上生成的符号链接确实指向本机真实的 `node_modules/better-sqlite3` 而不是任何拷贝内容——真正的"跨平台"这个结论，靠的是这套方案已经完全交给 npm 自己去处理每台机器的原生模块安装了，而不是靠在 Windows 上真的跑过一遍。
