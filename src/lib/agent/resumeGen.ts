...
export type PersonalInfoForPrompt = {
  ...
  /** The user's persisted, resume-wide skills list (see the Skill model) —
   * the ONLY source for the resume's Skills section; see the data-isolation
   * rule in SYSTEM_PROMPT below. */
...
You will be given: the user's personal info (including optional GitHub/LinkedIn — include them in the header only if non-empty, and a persisted skills list — see below), a job description (JD, possibly as a URL you must fetch with WebFetch), a library of the user's past experiences, and a Typst template to use as a style/layout reference. Each experience (e.g. one internship) has its own startDate/endDate and optional location (e.g. "Pittsburgh, PA") and contains one or more independently STAR-Q'd highlights, each with its own role tags and a pre-written resumeBullet. Use the experience's own startDate/endDate for its date range on the resume — don't write a placeholder like "[Dates Not Provided]" when they're actually given. Include an experience's location alongside its dates/title only if the template's own layout already has a slot for it and the location is non-empty — don't invent a new layout element just to fit it in.

The personal info's skills array is the user's persisted, curated skills list (name + optional category). If the template has a Skills section, populate it using ONLY entries from this list — select and group the ones most relevant to the JD (using the given category as a grouping hint where present), but never add a tool/language/framework/platform that isn't in this list, even if it seems like a natural fit for the JD or is implied by an experience's description. If the skills list is empty, leave the Skills section as an empty/placeholder state rather than inventing content for it.

Your job:
1. Read the JD and identify the target role and the 4-6 most relevant skills/keywords.
2. Select individual highlights (not whole experiences) whose tags/content best match the JD — prefer quantified, relevant results over recency when deciding WHICH highlights to include. An experience may contribute all, some, or none of its highlights. Keep the highlights you use grouped under their parent experience/org on the resume. Within a section (e.g. Work Experience, Projects), order the experience/org entries themselves in reverse-chronological order (most recent startDate first) — never by relevance — e.g. an internship in 2026 is listed above one from 2025 even if the earlier one's highlights are individually more relevant to this JD. Relevance only governs which highlights get selected and how they're worded, not the entries' left-to-right or top-to-bottom ordering.
